...
mixin EventViewStateMixin on State<EventView>, TickerProviderStateMixin<EventView>,
    EventViewAnimationsMixin<EventView>, EventViewVotingMixin<EventView>,
    EventViewTranslationMixin {
  EventModel? event;

  void initialize_state() {
    on_voting_card_visibility_changed = (bool show, bool enable) {
      if (mounted) setState(() { show_voting_card = show; enable_voting_card = enable; });
    };
    get_event_from_path();
    check_if_user_voted();
  ...
  void get_event_from_path() async {
    ...
    try {
      ...
      if (!event_doc.exists) { debugPrint('Event not found: $event_id'); return; }
  ...
  Future<void> _retry_get_event() async {
    ...
    } catch (e) { debugPrint('Retry failed - Error getting event: $e'); }
  }
